...
def test_failed_result_output():
    result = RevisionResult(revision="002", passed=False, failures=["Row lost in users table"])
    out = _capture(print_revision_result, result)
    assert "002" in out
...
def test_skipped_result_output():
    result = RevisionResult(
        revision="003", passed=True, skipped=True, skip_reason="intentional data migration"
    )
    out = _capture(print_revision_result, result)
    assert "003" in out
...
def test_summary_with_failures():
    results = [
        RevisionResult(revision="001", passed=True),
        RevisionResult(revision="002", passed=False, failures=["Data lost in orders"]),
    ]
    out = _capture(print_check_all_summary, results)
    assert "002" in out
...
def test_risk_score_scales_with_failures():
    r = RevisionResult(revision="x", passed=False, failures=["a", "b", "c"])
    assert r.risk_score == 75


def test_risk_score_capped_at_100():
    r = RevisionResult(revision="x", passed=False, failures=["a"] * 10)
    assert r.risk_score == 100


def test_failure_summary_format():
    r = RevisionResult(revision="x", passed=False, failures=["Table lost", "Column missing"])
